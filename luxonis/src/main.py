...
from detectors.pose import getKeypoints, getValidPairs, getPersonwiseKeypoints
import cv2
...
def create_pipeline():
    ...
    if args.camera:
        pose_nn.setBlobPath(str((Path(__file__).parent / Path("../models/human-pose-estimation-0001_openvino_2021.2_6shave.blob")).resolve().absolute()))
    else:
        pose_nn.setBlobPath(str((Path(__file__).parent / Path("../models/human-pose-estimation-0001_openvino_2021.2_8shave.blob")).resolve().absolute()))
